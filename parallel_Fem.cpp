...
mesh mesh_generator_1D(std::vector<double> interval, int n)
{
    ...
    m.p = p;
    m.t = t;
    double h =  (1 / static_cast<double>(n)) * (interval[1] - interval[0]);

    #pragma omp parallel for num_threads(4) schedule(guided)
    for (int i = 0; i < n + 1; i++)
    {
        m.p[i] = interval[0] + (i*h);
                                         ...
                                         double tol = 1e-10)
{
    ...
    for (int k = 0; k < max_iter; ++k)
    {
        ...
        double dot_pAp = 0.0;
        #pragma omp parallel for reduction(+:dot_pAp)  schedule(static)
        for (int i = 0; i < n; ++i)
        {
            dot_pAp += p[i] * Ap[i];
        }
